...
module Rack
  ...
  class Request
    ...
    @forwarded_priority = %i[forwarded x_forwarded]
    @x_forwarded_proto_priority = %i[proto scheme]
    ...
    self.ip_filter = ->(ip) { trusted_proxies.match?(ip) }
    ...
    module Helpers
      ...
      def query_string = get_header(QUERY_STRING).to_s
      def content_length = get_header("CONTENT_LENGTH")
      def logger = get_header(RACK_LOGGER)
      def user_agent = get_header("HTTP_USER_AGENT")

      # the referer of the client
      def referer = get_header("HTTP_REFERER")
      alias referrer referer

      def session
        fetch_header(RACK_SESSION) do |_k|
          set_header RACK_SESSION, default_session
        end
      end

      def session_options
        fetch_header(RACK_SESSION_OPTIONS) do |_k|
      ...
      def trace? = request_method == TRACE
      ...
      def ip
        ...
        return external_addresses.last unless external_addresses.empty?
      ...
      def POST
        ...
        begin
          ...
          if (form_hash = get_header(RACK_REQUEST_FORM_HASH)) && get_header(RACK_REQUEST_FORM_INPUT).equal?(rack_input)
          ...
          set_header(RACK_REQUEST_FORM_ERROR, e)
          raise
        end
      end
      ...
      # <tt>env['rack.input']</tt> is not touched.
      def delete_param(k)
        post_value = self.POST.delete(k)
        get_value = self.GET.delete(k)
      ...
      def [](key)
        warn(
          "Request#[] is deprecated and will be removed in a future version of Rack. Please use request.params[] instead", uplevel: 1
        )
      ...
      def parse_http_accept_header(header)
        header.to_s.split(/\s*,\s*/).map do |part|
          ...
          quality = 1.0
          quality = ::Regexp.last_match(1).to_f if parameters and /\Aq=([\d.]+)/ =~ parameters
          [attribute, quality]
        end
      end
      ...
      def split_authority(authority)
        ...
        [match[:host], match[:address], match[:port]&.to_i]
      end
        ...
        forwarded_priority.each do |type|
          ...
          when :x_forwarded
            x_forwarded_proto_priority.each do |x_type|
              next unless header = FORWARDED_SCHEME_HEADERS[x_type]

              split_header(get_header(header)).reverse_each do |scheme|
                return scheme if allowed_scheme(scheme)
              end
            end
          end
        end
